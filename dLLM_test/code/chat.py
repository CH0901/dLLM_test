...
def chat(args):
    device = 'cuda'
    model_path = args.model_path or 'GSAI-ML/LLaDA-8B-Instruct'
    ...
    if args.quant_ckpt:
        import sys; sys.path.insert(0, '/content/Fast-dLLM/llada')
        from apply_duquant import apply_duquant_to_model
        print(f"[DuQuant] loading {args.quant_ckpt}")
        params = torch.load(args.quant_ckpt, map_location='cpu')
        model  = apply_duquant_to_model(model, params, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--quant_ckpt', type=str, default=None)
